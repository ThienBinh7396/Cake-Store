...
class CategoryController{
    ...
    if(check){
      ...
      .then(rs => {
        if(!rs){
          ...
          res.send(helper.getStatus('success', 'Create category successfully', rs));
        }
      })
      .catch(err => {
        console.log(err);
  ...
  async update(req, res){
    ...
    if(check){
      ...
      .then(category => {
        if(!category){
          res.send(helper.getStatus('error', `Can't find category with identity ${provideAttributes.id}`));
        }else{
          category.update({
            title: provideAttributes.title,
            alias: provideAttributes.alias
          ...
          .then(rs => {
            if(rs){
              res.send(helper.getStatus('success', 'Update category successfully', rs));
            }else{
  ...
  delete(req, res){
    ...
    .then(rs =>{
      if(!rs){
        ...
        .then(rs => {
          res.send(helper.getStatus('success', 'Delete categories successfully'))
        })
        .catch(err => {
          res.send(helper.getStatus('error', 'Delete categories failed'))
        })
