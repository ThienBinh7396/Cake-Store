...
class CakeCategoriesController{
  ...
  async updateGallery(productId, _gallery){
    return new Promise(res => {
      ...
      .then(rs => {
        Gallery.bulkCreate(_gallery.map(it => {
          return {
            product_id: productId,
            url: it.url
          }
  ...
  create(req, res){
    let providerAttributes = helper.checkPostProviderAttributes(req, res, ['title', 'price', 'discount', 'description', 'status', 'thumbnail', 'gallery']);
    ...
    .then(async rs => {
      ...
      if(!rs){
        ...
        let result = await this.updateGallery(product.id, providerAttributes.gallery);

        if(result){
          res.send(helper.getStatus('success', 'Create product successful!', await this.helperFindOne(product.id)));
        }else{
  ...
  update(req, res){
    ...
    .then(_product => {
      if(!_product){
        ...
        let {title, price, discount, description, status, thumbnail, gallery} = req.body;
        ...
        .then(async  _update => {
          if(gallery){
            let updateGallery = await this.updateGallery(product.id, gallery);
            if(!updateGallery){
              return res.send(helper.getStatus('error', 'Update product failed!'));
            }
          }
          return res.send(helper.getStatus('success', 'Update product successful!', await this.helperFindOne(product.id)));
